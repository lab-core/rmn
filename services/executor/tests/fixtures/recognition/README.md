...
- `grades_*.png`: the "Réservé" grade table of a cover page, cropped exactly
  ...
  7.0` overlay whose jpeg-degraded dots touch the digits: the model misreads
  it (known miss). `grades_printed_decimals.png` is not a scan: the grades
...
- `expected.json`: the page shape and boxes the crops were taken with, and
  ...
  crops the current model misreads (kept so an improvement shows up as an
  unexpected pass, not so the misread is asserted).
