...
Pull the images into the node before an apply or a rollout when the network to
Docker Hub is slow: the kubelet pulls one image at a time, so a slow pull of the
1.5 GB executor image holds every other pod (redis included) in
`ContainerCreating` until it ends. `./minikube-helper.sh -p` pulls every image
the manifests reference with `minikube image pull`; with the images on the node
the kubelet only compares digests at rollout time.
...
into both Ingress rules (`deployment/replacements.yaml`) and the server and
socketIO Deployments read it for their CORS allow-lists (`CORS_ORIGINS`,
`SOCKETIO_CORS_ORIGINS`; unset, both accept every origin). To deploy against another host
...
##### Create a user
Role can be either "Utilisateur" or "Administrateur"; passwords need 8 to 32
